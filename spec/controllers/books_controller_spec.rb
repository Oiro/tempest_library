require 'rails_helper'

RSpec.describe BooksController, type: :controller do
  it "assigns a book" do
       get :new
       expect(assigns(:book)).to be_a_new(Book)
  end
  context "POST#CREATE" do
      before(:each) do
       post :create, params: { book: { title: "Beginning Ruby", isbn: 4567, user_id: 1, quantity: 67 } }

      end
      
      it "saves the book to the database" do
       pending
       before_count = Book.count

       expect(Book.count).not_to eq(before_count)

      end
      it "displays the correct flash" do
       expect(flash[:notice]).to have_content("Book successfully added")
      end
      it "redirects to the book index" do
       expect(response).to redirect_to(books_url)
      end
  end

  context "GET#EDIT" do

     it "returns an html form for editing" do
      book = FactoryGirl.create(:book)
      get :edit, params: {id: book}
      expect(response).to have_http_status(:success)
      expect(response).to render_template(:edit)
     end
  end

  context "PUT#UPDATE" do
     it "updates element details in the database" do
       book = FactoryGirl.create(:book)   
       put :update, params: {  book:  { :title => "Functional Aspects of the IDE Environment" }   }
       expect(Book.where(title: "Functional Aspects of the IDE Environment")).to be_present
       expect(Book.where(title: "Functional Aspects of the IDE Environment")).not_to eq("Beginning Ruby")
     end

  end

  context "GET#SHOW" do
    it "returns an html form for displaying results" do
      get :show
      expect(response).to have_http_status(:success)
      expect(response).to render_template(:show)
    end


  end
end
